...
int main(int argc, char* argv[]) {
  ...
  for (size_t t = 0; t < T; t++) {
    ...
    // continuousely wait for incomming positions from all other ranks
    // since each rank sends to self, that request should come in
    // almost instantly and we can already start computing. this could
    // be considered to be some kind of latency hiding.
    // the other requests will come in later.
    for (size_t recv_cnt = 0; recv_cnt < R; recv_cnt++) {

      // wait for any request to come in
      // the variable r will hold the index of the rank from which
      // the particles were received
      int r;
      MPI_Waitany(R, recv_reqs, &r, MPI_STATUS_IGNORE);

      // compute the interaction between the local elements and the elements
      // that were received. we can do this, since the interaction is a linear
      // combination. alternatively, we could also use an array for the forces.
      for (size_t k = 0; k < K; k++) {

        // the offset in the global array
        ...
        // iterate over the received elements
        for (size_t j = K * r; j < K; j++) {

          // do not compute interaction with self
          if (i == j) {
            continue;
          }
    ...
    // finally, update the local positions using the updated velocities
  //  for (size_t k = 0; k < K; k++) {
  //    local_particles[k].p.x += local_particles[k].v.x;
  //    local_particles[k].p.y += local_particles[k].v.y;
  //    local_particles[k].p.z += local_particles[k].v.z;
 //   }
    ...
    // make sure that we wait for the send calls to complete
